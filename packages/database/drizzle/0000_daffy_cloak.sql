...
CREATE TYPE "public"."field_type" AS ENUM('short_text', 'long_text', 'number', 'email', 'phone', 'date', 'select', 'multi_select', 'radio', 'checkbox', 'file', 'rating', 'scale');--> statement-breakpoint
...
CREATE TABLE "auths" (
	...
	"email_otp_verification" varchar(10),
	"email_otp_verification_expiry" timestamp,
	"otp_verification_secret" varchar(255),
...
CREATE TABLE "form_comments" (
	"form_comment_id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"form_id" uuid NOT NULL,
	"user_id" uuid,
	"guest_token" uuid,
	"text" text NOT NULL,
...
ALTER TABLE "form_comments" ADD CONSTRAINT "form_comments_user_id_users_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("user_id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "form_comment_form_id_idx" ON "form_comments" USING btree ("form_id");--> statement-breakpoint
