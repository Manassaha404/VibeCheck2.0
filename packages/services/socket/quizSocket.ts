...
async function subscribe() {
  await subscriber.subscribe(PARTICIPANT_JOIN_CHANNEL_NAME, EMIT_QUESTION_CHANNEL_NAME, SUBMIT_ANSWER_CHANNEL_NAME, ACTIVATE_SESSION_CHANNEL_NAME,REVEAL_ANSWER_CHANNEL_NAME, END_SESSION_CHANNEL_NAME);
}
...
export function registerQuizSocket(io: Server) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join:participant:session", async({ sessionId }) => {
      ...
      await publisher.publish(PARTICIPANT_JOIN_CHANNEL_NAME, JSON.stringify({sessionId}))
    });
    socket.on("emit:question", async({ sessionId, questionPayload, questionIndex }) => {
      await publisher.publish(EMIT_QUESTION_CHANNEL_NAME, JSON.stringify({sessionId, questionIndex, questionPayload}))
    });
    socket.on(
      "submit:answer",
      async ({ sessionId, optionIds, questionId, text, userId }) => {
        await publisher.publish(SUBMIT_ANSWER_CHANNEL_NAME, JSON.stringify({sessionId, optionIds, questionId, text, userId}))

        if (optionIds && questionId && userId) {
          try {
            await quizService.recordAnswer(userId, { sessionId, questionId, optionIds });
          } catch (err) {
    ...
    socket.on("activate:session", async({ sessionId }) => {
      await publisher.publish(ACTIVATE_SESSION_CHANNEL_NAME, JSON.stringify({sessionId}))
    });    
    socket.on("reveal:answer", async({ sessionId, questionId, correctOptionIds }) => {
      await publisher.publish(REVEAL_ANSWER_CHANNEL_NAME, JSON.stringify({sessionId, questionId, correctOptionIds}))
    });

    socket.on("end:session", async({ sessionId }) => {
      await publisher.publish(END_SESSION_CHANNEL_NAME, JSON.stringify({sessionId}))
    });
