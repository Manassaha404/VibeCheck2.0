...
export type {
  MoodFieldAnalytics as MoodFieldData,
...
const getMoodDisplay = (
  ...
  if (options && options.length > 0) {
    const opt = options.find(
      (o) => o.id === key || o.value.toLowerCase() === key.toLowerCase(),
    );
    if (opt) {
      return { emoji: opt.id, label: opt.value };
    }
  }
  return MOOD_DISPLAY[key.toLowerCase()] ?? { emoji: key, label: key };
};
        ...
        <div className="flex flex-col md:flex-row items-center gap-12 w-full">
          ...
          >
            <div className="w-28 h-28 bg-pure-white border-4 border-ink-charcoal rounded-full flex flex-col items-center justify-center shadow-[inset_4px_4px_0px_0px_rgba(44,46,42,1)]">
              <span className="text-3xl leading-none">
                {topMood
                  ? getMoodDisplay(topMood.mood, moodField?.options).emoji
                  : "🤔"}
              </span>
          ...
          <div className="w-full flex flex-col gap-4 font-bold uppercase text-base flex-1">
            {moodField.distribution.map((entry, i) => {
              const display = getMoodDisplay(entry.mood, moodField.options);
              return (
                <div
                  key={entry.mood}
                  className="flex items-center gap-4 bg-canvas-cream p-4 border-2 border-ink-charcoal shadow-hard-sm"
                >
                  <div
                  ...
                  <span className="text-lg">
                    {display.label} ({entry.pct}%)
                  </span>
                  <span className="ml-auto text-lg opacity-80">
