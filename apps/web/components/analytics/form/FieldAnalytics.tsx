  ...
  return (
    <div className="flex-grow space-y-4 overflow-y-auto max-h-[300px] pr-2 custom-scrollbar">
      {data.samples.map((sample, i) => (
        <div
          key={i}
          className={`p-4 ${BUBBLE_STYLES[i % BUBBLE_STYLES.length]}`}
        >
          {sample}
        </div>
      ))}
        ...
        {data.distribution.map((entry, i) => {
          ...
          return (
            <div key={entry.label} className="flex items-center gap-2">
              <span className="w-14 text-xs font-black text-right shrink-0">
                {entry.label}
              </span>
    ...
    <div className="space-y-4 flex-grow">
      {data.distribution.map((entry, i) => (
        <div key={entry.mood} className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold w-12 text-right opacity-70">
            {entry.count}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-grow py-8 gap-4">
      ...
      <div className="bg-electric-sun border-4 border-ink-charcoal p-6 text-center shadow-[8px_8px_0px_0px_rgba(44,46,42,1)]">
        <span className="font-display-lg text-[64px] font-black leading-none">
          {data.totalUploads}
        </span>
...
function renderAnalytics(analytics: FieldAnalyticsData) {
  switch (analytics.kind) {
    case "text":
      return <TextCard data={analytics} />;
    case "choice":
      return <ChoiceCard data={analytics} />;
    case "numeric":
      return <NumericCard data={analytics} />;
    case "mood":
      return <MoodCard data={analytics} />;
    case "file":
      return <FileCard data={analytics} />;
    case "date":
      return <DateCard data={analytics} />;
  }
}
...
function getSubtitle(analytics: FieldAnalyticsData): string {
  switch (analytics.kind) {
    case "text":
      return `${analytics.totalAnswered} recent entries`;
    case "choice":
      return `${analytics.totalAnswered} total selections`;
    case "numeric":
      return `${analytics.totalAnswered} responses`;
    case "mood":
      return `${analytics.totalAnswered} mood responses`;
    case "file":
      return `${analytics.totalUploads} file uploads`;
    case "date":
      return `${analytics.totalAnswered} date entries`;
  }
}
