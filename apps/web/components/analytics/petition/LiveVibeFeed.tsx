...
export default function LiveVibeFeed({
  signatures,
}: {
  signatures: {
    firstName: string;
  ...
  const getBadgeAndIcon = (index: number) => {
    const badges = [
      ...
      {
        badge: "Loud Supporter",
        bg: "bg-[var(--color-electric-sun)]",
        icon: <Megaphone size={16} />,
      },
      {
        badge: "Cool",
        bg: "bg-[var(--color-pure-white)]",
  ...
  return (
    <section className="bg-[var(--color-pure-white)] border-2 border-[var(--color-ink-charcoal)] p-6 shadow-hard h-full min-h-[500px] flex flex-col overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-6 border-b-2 border-[var(--color-ink-charcoal)] pb-4 shrink-0 relative z-10 bg-[var(--color-pure-white)]">
        <h3 className="font-display text-headline-sm uppercase">
          Live Vibe Feed
        </h3>
        <Rss
          className="text-[var(--color-leaf-green)] animate-[pulse-border_2s_infinite] rounded-full"
          size={24}
        />
      </div>
        ...
        <div className="absolute top-0 left-0 right-0 w-full animate-scroll-vertical space-y-4 pb-4">
          ...
          {feedItems.length > 0 ? (
            [...feedItems, ...feedItems].map((item, idx) => (
              <div
                key={`${item.id}-${idx}`}
                className="border-2 border-[var(--color-ink-charcoal)] p-3 bg-[var(--color-canvas-cream)] flex items-start gap-3 hover:scale-[1.02] transition-transform cursor-pointer shadow-hard-sm"
              >
                <div
                  className={`${item.bg} border-2 border-[var(--color-ink-charcoal)] p-2 rounded-full mt-1 shrink-0`}
                >
                  {item.icon}
                </div>
                <div>
                  <p className="font-body text-label-md font-bold">
                    {item.name}{" "}
                    <span className="text-xs font-normal text-[var(--color-outline)] ml-2">
                      {item.time}
                    </span>
                  </p>
                  {item.text && (
                    <p className="font-body text-body-md text-sm mt-1 text-[var(--color-ink-charcoal)]">
                      {item.text}
                    </p>
                  )}
                  <span className="inline-block bg-[var(--color-pure-white)] border border-[var(--color-ink-charcoal)] px-2 py-0.5 mt-2 text-[10px] font-bold uppercase">
                    {item.badge}
                  </span>
                </div>
              </div>
            ...
            <p className="font-body text-label-md text-[var(--color-outline)] text-center py-8">
