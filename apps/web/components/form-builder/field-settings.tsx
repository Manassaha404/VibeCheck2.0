...
export function FieldSettings() {
  const { nodes, selectedNodeId, updateNodeData, removeNode } =
    useFormBuilderStore();
      ...
      <div className="p-6 overflow-y-auto space-y-8 flex-grow">
        ...
        <div className="space-y-2">
          <label className="text-label-md font-label-md uppercase text-outline flex items-center gap-1">
            Question Label
            <span title="The main text displayed above the input">
            ...
            <input
              ...
              placeholder={(() => {
                switch (data.type) {
                  ...
                  case "long_text":
                    return "Enter detailed text here...";
                  case "number":
                    return "123";
                  case "email":
            ...
            <div className="flex flex-col gap-3 p-4 border-2 border-ink-charcoal rounded-DEFAULT bg-canvas-cream">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-ink-charcoal text-label-md">
                    Google Drive
                  </span>
                  <span className="text-ink-charcoal/70 text-[11px] leading-tight">
                    Send uploaded files directly to your Drive
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center justify-between cursor-pointer group">
            ...
            <div className="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
              ...
              />
              <div
                className={`toggle-label block overflow-hidden h-6 rounded-full border-2 border-ink-charcoal cursor-pointer ${data.isRequired ? "bg-leaf-green" : "bg-surface-variant"}`}
              ></div>
            </div>
          </label>
            ...
            <label className="flex items-center justify-between cursor-pointer group pt-2 border-t-2 border-ink-charcoal/10">
              <div className="flex items-center gap-1">
                <span className="text-label-md font-label-md flex items-center gap-2">
                  Primary Field
                  <span title="Used as the main identifier for submissions (e.g. Drive Folder name). Only one field can be Primary.">
                    <Info className="w-4 h-4 text-ink-charcoal/50 cursor-help" />
                  </span>
                </span>
              </div>
              <div className="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in">
                ...
                />
                <div
                  className={`toggle-label block overflow-hidden h-6 rounded-full border-2 border-ink-charcoal cursor-pointer ${data.isPrimary ? "bg-electric-sun" : "bg-surface-variant"}`}
                ></div>
              </div>
            </label>
          )}
