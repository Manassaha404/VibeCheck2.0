...
export function FormLinearPreview() {
  const { nodes, selectedNodeId, setSelectedNode, reorderNodes } =
    useFormBuilderStore();
        ...
        <div className="w-full max-w-2xl space-y-6 pb-32">
          {nodes.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-12 border-4 border-dashed border-ink-charcoal/30 rounded-xl bg-pure-white text-ink-charcoal/50">
              <p className="text-headline-sm font-headline-sm font-bold">
                No fields yet
              </p>
              <p className="text-body-lg font-body-lg">
                Drag or click fields from the palette to get started.
              </p>
            </div>
          ) : (
            <DndContext
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        {data.helperText && (
          <p className="text-body-md text-ink-charcoal/60 mb-4">
            {data.helperText}
          </p>
        )}
