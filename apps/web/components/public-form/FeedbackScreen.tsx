...
export function FeedbackScreen({
  ...
  const config = {
    expired: {
      ...
      title: "Form Expired",
      message:
        "This form is no longer accepting responses because it has passed its expiration date.",
      color: "bg-[var(--color-electric-sun)]",
    },
    limit_reached: {
      icon: (
        <AlertTriangle size={48} className="text-[var(--color-ink-charcoal)]" />
      ),
      title: "Limit Reached",
      message:
        "This form has reached its maximum number of responses and is now closed.",
      color: "bg-[#FF9B9B]",
    },
    already_responded: {
      icon: (
        <CheckCircle2 size={48} className="text-[var(--color-ink-charcoal)]" />
      ),
  ...
  return (
    <div className="bg-white border-[3px] border-[var(--color-ink-charcoal)] shadow-hard-lg w-[90%] md:w-full max-w-md p-6 md:p-12 text-center relative z-10 flex flex-col items-center justify-center animate-pop-in mx-auto mt-12 md:mt-20">
      ...
      {(!allowEdit || state !== "already_responded") && (
        <Link
          href="/"
          className="inline-block mt-4 text-[var(--color-ink-charcoal)] font-bold underline underline-offset-4 hover:text-[var(--color-electric-sun)] transition-colors"
        >
          Return to VibeCheck
        </Link>
      )}
