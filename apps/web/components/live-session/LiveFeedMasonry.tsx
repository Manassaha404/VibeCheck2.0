...
export default function LiveFeedWordCloud({ messages }: LiveFeedWordCloudProps) {
  ...
  const maxCount = groupedTexts.length > 0 ? Math.max(...groupedTexts.map(item => item.count)) : 1;
    ...
    <section className="mt-12 flex flex-col gap-8 w-full">
      <div className="flex justify-between items-center bg-[var(--color-canvas-cream)] border-4 border-[var(--color-ink-charcoal)] px-6 py-3 shadow-hard inline-block w-max mx-auto">
        <h3 className="text-headline-sm font-display font-black text-[var(--color-ink-charcoal)] uppercase flex items-center gap-2">
          <Rss className="text-[var(--color-leaf-green)] animate-pulse" size={32} strokeWidth={3} />
          LIVE ANSWERS
        </h3>
      </div>
            ...
            return (
              <span 
                ...
                style={{ 
                  fontSize: `${size}rem`,
                  lineHeight: 1.1,
                  WebkitTextStroke: size > 2 ? '2px var(--color-ink-charcoal)' : '1px var(--color-ink-charcoal)',
              ...
              >
                {item.text}
                {item.count > 1 && (
                  <sup className="text-label-sm ml-1 text-[var(--color-ink-charcoal)]" style={{ WebkitTextStroke: '0' }}>
                    x{item.count}
                  </sup>
                )}
              </span>
