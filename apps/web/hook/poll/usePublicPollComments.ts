...
export function usePublicPollComments({
  username,
  slug,
  pollId,
}: UsePublicPollCommentsProps) {
  const trpcUtils = trpc.useUtils();
  const { data: comments, isLoading } =
    trpc.poll.getPublicPollCommentsBySlug.useQuery(
      { username, slug },
  ...
  useEffect(() => {
    ...
    const handleVoteUpdate = (payload: any) => {
      ...
      if (
        eventPollId === pollId &&
        pollData?.comment &&
        pollData.comment.trim() !== ""
      ) {
        trpcUtils.poll.getPublicPollCommentsBySlug.setData(
          { username, slug },
          (oldData) => {
            const newComment = {
