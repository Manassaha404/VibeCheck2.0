...
import type {
  ExportField,
  TimePeriod,
} from "@/components/analytics/form/ExportSection";
...
export function useFormExport({ formSlug, fields }: UseFormExportProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      do {
        const result = await utils.form.getFormResponses.fetch({
          formSlug,
          page,
          limit,
        });
      ...
      const exportFields = apiFields.filter((f) =>
        selectedFields.has(f.fieldId),
      );
