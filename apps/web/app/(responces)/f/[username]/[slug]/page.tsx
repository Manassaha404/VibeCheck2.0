...
export default function PublicFormPage({ params }: PageProps) {
  ...
  const { form, isLoading, isError, session, isFetching } = usePublicForm(
    username,
    slug,
    {
      password,
      ...
      >
        ...
        {isAgentComplete ? (
          ...
          >
            ...
            {/* Content Area */}
            <div
              className={`flex flex-col ${mode === "agent" && !isAgentComplete ? "flex-1 min-h-0" : "flex-1 min-h-[300px]"}`}
            >
              {mode === "agent" ? (
                <div className="flex-1 flex flex-col min-h-0 animate-pop-in">
                  <AgentChatPanel
                    ...
                    primaryFieldId={
                      form.fields.find((f: any) => f.isPrimary)?.fieldId
                    }
                  />
                </div>
              ) : (
