...
export default function FormEditPage({ params }: PageProps) {
  ...
  const { form, isLoading, isError, isFetching } = usePublicForm(
    username,
    slug,
    {
      password,
  ...
  // ── Error / Not Found ────────────────────────────────────────────────────
  if (isError || !form) {
    return <ContentErrorState kind="form" />;
  }
    ...
    return (
      <div className="min-h-screen bg-[var(--color-canvas-cream)] flex flex-col">
        <FeedbackScreen
          state={form.access}
          allowEdit={
            "allowResponseEdit" in form ? form.allowResponseEdit : false
          }
          onEdit={() => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-canvas-cream)] flex flex-col text-[var(--color-ink-charcoal)] selection:bg-[var(--color-electric-sun)] selection:text-[var(--color-ink-charcoal)]">
      {/* ── Header ──────────────────────────────────────────────────────── */}
      <PublicFormHeader
        mode="form"
