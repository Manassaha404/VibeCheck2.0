...
export default function EditQuizPage() {
  ...
  const { data, isLoading, isError } = trpc.quiz.getQuizForEdit.useQuery({ quizId }, {
  ...
  useEffect(() => {
    if (data && !isInitialized) {
      ...
      const mappedQuestions = data.questions.map((q) => ({
        ...
        type: (q.isTextAnswer ? "text_entry" : "multiple_choice") as QuestionType,
        text: q.text,
        options: (q.options || []).map(opt => ({
          ...opt,
          id: opt.id || `opt-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`
        })),
  ...
  if (isLoading || !isInitialized) {
    return (
      <div className="bg-canvas-cream text-ink-charcoal font-body min-h-screen flex flex-col justify-center items-center bg-dot-pattern selection:bg-electric-sun selection:text-ink-charcoal">
        ...
        <h2 className="font-display text-headline-md">Loading Quiz Editor...</h2>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="bg-canvas-cream text-ink-charcoal font-body min-h-screen flex flex-col justify-center items-center bg-dot-pattern selection:bg-electric-sun selection:text-ink-charcoal">
        <h2 className="font-display text-headline-md text-red-500 mb-4">Error loading quiz</h2>
        <Link href="/dashboard" className="underline font-bold text-lg">Go back to dashboard</Link>
      </div>
    );
  }
