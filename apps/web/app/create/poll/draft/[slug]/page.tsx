...
export default function PollDraftPage({
  params,
}: {
      ...
      setDraft({
        ...
        options:
          options && options.length > 0
            ? options.map((o: any) => ({
                id: o.pollOptionId || String(o.orderIndex),
                text: o.text,
              }))
            : [
                { id: "1", text: "Option 1" },
    ...
    <div className="bg-canvas-cream text-ink-charcoal min-h-screen flex flex-col font-body-lg antialiased bg-dot-pattern">
      ...
      {isLoading && (!currentPoll || currentPoll.slug !== slug) ? (
        <div className="flex flex-col items-center justify-center flex-grow">
          ...
          <p className="mt-4 font-headline-sm text-ink-charcoal animate-pulse">
            Loading draft...
          </p>
        </div>
        ...
        <div className="flex-grow flex flex-col items-center justify-center p-4 md:p-8 max-w-2xl mx-auto text-center mt-[-5vh] md:mt-[-10vh]">
          ...
          <h2 className="text-4xl md:text-display-lg text-ink-charcoal mb-4 md:mb-6 leading-tight">
            Already Airborne!
          </h2>
          <p className="text-base md:text-body-lg text-ink-charcoal font-bold mb-8 md:mb-10 max-w-md mx-auto px-4 md:px-0">
            This poll has already been published. Drafts cannot be edited once
            they're live in the wild.
          </p>
          <Link
